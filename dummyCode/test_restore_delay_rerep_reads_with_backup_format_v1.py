...
class TestRestoreDelayRerepReadsWithBackup(RestoreDelayRerepTestSuite):
    """
    Test to validate rerep after a restore, in the presence of events that
    delay rerep reads. Additionally, we also issue a backup during the
    rerep and validate that it completes
    ...
    def modify_test_dimensions(cls):
        gucs = BARTestSuite.backup_restore_configs_v1()
        gucs['disk_mirror_count'] = ['0', '1']
        return Dimensions(gucs)

    def test_restore_delay_rerep_reads_with_backup(self, cluster,
        ...
        self.restore_delay_rerep_reads_with_backup(cluster, cluster_session,
                                                   db_session, gucs)
