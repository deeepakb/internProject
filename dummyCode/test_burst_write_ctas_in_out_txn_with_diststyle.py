...
class TestBurstWriteCtasInOutTxnSameSliceDiststyleSS(TestBurstWriteCTASInTxn):
    @classmethod
    def modify_test_dimensions(cls):
        return Dimensions(
            dict(
                ...
                in_txn=[True, False],
                sortkey=[''],
                temp_referenced=[False],
                diststyle=['', 'diststyle even', 'distkey({})']))

    def test_burst_write_ctas_in_out_txn_same_slice_diststyle_SS(
...
@pytest.mark.usefixtures("super_simulated_mode")
class TestBurstWriteCtasInOutTxnDiffSliceDiststyleSS(TestBurstWriteCTASInTxn):
    @classmethod
    def modify_test_dimensions(cls):
        return Dimensions(
            dict(
    ...
    def test_burst_write_ctas_in_out_txn_diff_slice_diststyle_SS(
            self, db_session, cluster, vector, is_temp):
        """
        This test runs CTAS on Burst in txn when the number of
