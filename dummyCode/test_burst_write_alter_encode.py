...
class TestBurstWriteAlterEncode(AlterTableSuite, BurstWriteTest):
    def _setup_tables(self, cursor, vector):
        diststyle = vector.diststyle
        sortkey = vector.sortkey
        tbl_def = (
            "create table dp20365_tbl(c0 int encode az64, c1 bigint) {} {}")
        tbl_def_burst = (
            "create table dp20365_tbl_burst(c0 int encode az64, c1 bigint) {} {}"
        )
        tbl_def_commit = ("create table dp20365_tbl_commit(c0 int)")
    ...
    def _alter_encode(self, cursor, table, col, encode):
        alter_ddl = ("ALTER TABLE {} ALTER COLUMN {} ENCODE {};").format(
            table, col, encode)
    ...
    def _dmls_abort(self, cluster, cursor, iteration_num, main_table,
                    burst_table):
        basic_insert = ("insert into {}(c0, c1) values({}, 111111111);")
        basic_insert_2 = ("insert into {}(c0, c1) values({}, 22222222222);")
        # Normal query
    ...
    def _validate_table_properties(self, cursor, schema, table):
        cursor.execute(
    ...
    def test_burst_write_alter_encode_commit(self, cluster, db_session,
                                             vector):
        """
        Test: burst read and write on table with alter encode.
        ...
        with db_session_master.cursor() as cursor, \
            ...
            # alter encode before burst write
            self._alter_encode(cursor, "dp20365_tbl", "c0", "raw")
            self._alter_encode(cursor, "dp20365_tbl_burst", "c0", "raw")
            self._start_and_wait_for_refresh(cluster)
            self._dmls_commit(cluster, cursor, 10, "dp20365_tbl",
                              "dp20365_tbl_burst")
            self._validate_table_data(cluster, cursor)
            # alter encode again after burst write
            self._alter_encode(cursor, "dp20365_tbl", "c0", "az64")
            self._alter_encode(cursor, "dp20365_tbl_burst", "c0", "az64")
            self._validate_table_data(cluster, cursor)
            self._validate_table_properties(cursor_bs, schema, "dp20365_tbl")
            self._validate_table_properties(cursor_bs, schema,
                                            "dp20365_tbl_burst")

    def test_burst_write_alter_encode_interleave_commit(
        ...
        with db_session_master.cursor() as cursor, \
            ...
            # begin burst write
            self._start_and_wait_for_refresh(cluster)
            self._dmls_commit(cluster, cursor, 6, "dp20365_tbl",
                              "dp20365_tbl_burst")
            self._validate_table_data(cluster, cursor)
            self._start_and_wait_for_refresh(cluster)
            self._dmls_commit(cluster, cursor, 6, "dp20365_tbl_burst",
                              "dp20365_tbl")
            self._validate_table_data(cluster, cursor)
            # alter encode in between
            self._alter_encode(cursor, "dp20365_tbl", "c0", "az64")
            self._alter_encode(cursor, "dp20365_tbl_burst", "c0", "az64")
            self._start_and_wait_for_refresh(cluster)
            self._dmls_commit(cluster, cursor, 6, "dp20365_tbl",
            ...
            self._validate_table_data(cluster, cursor)
            # alter encode again after burst write
            self._alter_encode(cursor, "dp20365_tbl", "c0", "lzo")
            self._alter_encode(cursor, "dp20365_tbl_burst", "c0", "lzo")
            self._validate_table_data(cluster, cursor)
            self._validate_table_properties(cursor_bs, schema, "dp20365_tbl")
            self._validate_table_properties(cursor_bs, schema,
                                            "dp20365_tbl_burst")

    def test_burst_write_alter_encode_abort(self, cluster, db_session, vector):
        """
        ...
        with db_session_master.cursor() as cursor, \
            ...
            # begin burst write
            self._start_and_wait_for_refresh(cluster)
            self._dmls_abort(cluster, cursor, 10, "dp20365_tbl",
                             "dp20365_tbl_burst")
            self._validate_table_data(cluster, cursor)
            # alter encode again after burst write
            self._alter_encode(cursor, "dp20365_tbl", "c0", "az64")
    ...
    def test_burst_write_alter_encode_interleave_abort(self, cluster,
                                                       db_session, vector):
        """
        Test: burst read and write on identity column.
        The test create two tables with the same identity columns.
        ...
        with db_session_master.cursor() as cursor, \
            ...
            # begin burst write
            self._start_and_wait_for_refresh(cluster)
            self._dmls_abort(cluster, cursor, 6, "dp20365_tbl",
                             "dp20365_tbl_burst")
            self._validate_table_data(cluster, cursor)
            self._start_and_wait_for_refresh(cluster)
            self._dmls_abort(cluster, cursor, 6, "dp20365_tbl_burst",
                             "dp20365_tbl")
            self._validate_table_data(cluster, cursor)
            # alter encode in between
            self._alter_encode(cursor, "dp20365_tbl", "c0", "az64")
            ...
            self._validate_table_data(cluster, cursor)
            # alter encode again after burst write
            self._alter_encode(cursor, "dp20365_tbl", "c0", "zstd")
            self._alter_encode(cursor, "dp20365_tbl_burst", "c0", "zstd")
            self._validate_table_data(cluster, cursor)
            self._validate_table_properties(cursor_bs, schema, "dp20365_tbl")
            self._validate_table_properties(cursor_bs, schema,
                                            "dp20365_tbl_burst")

    def test_burst_write_alter_encode_same_txn(self, cluster, db_session,
                                               vector):
        """
        Test: burst read and write on table with alter encode.
        The test create two tables with the same identity columns.
        ...
        the other one on burst cluster. They should contain same data at last.
        1. setup tables that are ready for burst.
        2. backup and refresh.
        3. alter encode and burst write in the same txn
        4. validate consistency of tables' content.
        """
        db_session_master = DbSession(cluster.get_conn_params(user='master'))
        schema = db_session_master.session_ctx.schema
        with db_session_master.cursor() as cursor, \
            ...
            self._alter_encode(cursor, "dp20365_tbl_burst", "c0", "raw")
            cursor.execute("set query_group to burst;")
            basic_insert = ("insert into {}(c0, c1) values(1, 111111111);")
            cursor.execute(basic_insert.format("dp20365_tbl"))
            cursor.execute(basic_insert.format("dp20365_tbl_burst"))
            self._check_last_query_didnt_burst(cluster, cursor)
            cursor.execute("commit")
            self._validate_table_properties(cursor_bs, schema,
                                            "dp20365_tbl_burst")

    def test_burst_write_alter_encode_same_txn_2(self, cluster, db_session,
                                                 vector):
        """
        Test: burst read and write on table with alter encode.
        The test create two tables with the same identity columns.
        ...
        1. setup tables that are ready for burst.
        2. backup and refresh.
        3. burst write, alter encode and burst write in the same txn
        4. validate consistency of tables' content.
        """
        db_session_master = DbSession(cluster.get_conn_params(user='master'))
        schema = db_session_master.session_ctx.schema
