...
log = logging.getLogger('test_burst_alter_distkey')

ALTER_STMT = "alter table customer_alter_distkey alter distkey c_nationkey"
...
# TPCH query #8
BURST_CANARY_QUERY = '''
select count(*) from customer_alter_distkey;
'''
...
class TestBurstAlterDistkey(BurstTest):
    ...
    def setup_data(self, db_session):
        ...
        load_table(db=db_session, dataset_name='tpch',
                   ...
                   table_name_suffix='_alter_distkey',
                   grant_to_public=True)

    def test_burst_alter_distkey_with_burst(
            self, db_session, cluster, verify_query_bursted):
        """
        Test runs ALTER DISTKEY on a table and confirms it can still
        ...
        with db_session.cursor() as cursor:
            ...
            cursor.execute("drop table if exists customer_alter_distkey")
            RedshiftClusterHelper.delete_snapshot(snapshot_identifier)

    def test_burst_alter_distkey_without_burst(
            self, db_session, cluster, verify_query_didnt_burst):
        """
        Test runs ALTER DISTKEY on a table and confirms it does not burst
