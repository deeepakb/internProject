...
class TestBurstWriteMaxConcurrencyBase(TestBurstWriteMVBase):
    ...
    def _run_select(self, cluster, cursor, tables, try_burst, should_burst,
                    no_burst_status=0):
        """
            try_burst - True if try to burst, else False
            should_burst - True if query should burst, else false

            Runs SELECT on the cluster
        """
    ...
    def _run_dmls(self, cluster, cursor, tables, try_burst, should_burst,
        ...
        for rdata in tables:
            ...
            self._copy_from_s3(cluster, cursor, table, S3_BUCKET)
            if try_burst and should_burst:
    ...
    def setup_tables(self, cluster, cursor, relprefix, diststyle, sortkey,
        ...
        for relid, t in enumerate(product(diststyle, sortkey)):
            ...
            cursor.execute(sql)
            self._unload_to_s3(cluster, cursor, relname, S3_BUCKET)
        ...
        if cluster.host_type == HostType.CLUSTER:
            cluster.release_all_burst_clusters()
            cluster.run_xpx('burst_update_max_clusters 5')
        # Save schema for debugging
        if clean_on_exit:
            cursor.execute(DROP_SCHEMA.format(MY_SCHEMA))

    def base_test_burst_write_max_concurrency(self, cluster):
        """
        ...
        """
        relprefix = "burst_write_max_concurrency"
        ...
        with self.setup_tables(cluster, cur1, relprefix, diststyle, sortkey,
            ...
            self._run_select(cluster, cur1, tables, True, True)
...
@pytest.mark.custom_local_gucs(gucs=dict(list(burst_write_basic_gucs.items()) + [
            ...
            ('burst_write_max_concurrent_queries', 99)
                            ]))
class TestBurstWriteMaxConcurrencySS(TestBurstWriteMaxConcurrencyBase):
    def test_burst_write_max_concurrency_ss(self, cluster):
        self.base_test_burst_write_max_concurrency(cluster)
...
class TestBurstWriteMaxConcurrencyCluster(TestBurstWriteMaxConcurrencyBase):
    def test_burst_write_max_concurrency_cluster(self, cluster):
        self.base_test_burst_write_max_concurrency(cluster)
