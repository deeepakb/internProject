...
class TestBarRenameBlockIDNonEncryptedNoRerepCluster4DiffAccount(
        TestBarRenameBlockID):
    def test_bar_rename_block_id_nonencrypted_no_rerep_cluster4_diff_account(
            self, cluster_session, cluster):
        self._backup_restore(
            cluster_session,
            cluster,
            encryptions="non_encrypted",
            same_account=False,
            rerep=False,
            target_cluster="CLUSTER4_A2_V2")
