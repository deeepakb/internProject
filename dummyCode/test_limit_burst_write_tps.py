...
NODE_TYPE = "event set EtTestNodeType,node_type={},max_nodes={}"
...
class TestLimitBurstWriteTPSBase(TestBurstWriteMVBase):
    ...
    def setup_tables(self, cluster, cursor, relprefix, diststyle, sortkey,
        ...
        cluster.run_xpx("event unset EtTestNodeType")
        # Get guc values
        max_nodes_ra3_xl = \
            cluster.get_guc_value('burst_write_max_num_nodes_ra3_xl')
        max_nodes_ra3_4xl = \
            cluster.get_guc_value('burst_write_max_num_nodes_ra3_4xl')
        max_nodes_ra3_16xl = \
            cluster.get_guc_value('burst_write_max_num_nodes_ra3_16xl')
        ...
        yield res
        # Restore guc values
        cluster.run_xpx(NODE_TYPE.format('i3en.xlarge', max_nodes_ra3_xl))
        cluster.run_xpx(NODE_TYPE.format('i3en.4xlarge', max_nodes_ra3_4xl))
        cluster.run_xpx(NODE_TYPE.format('i3en.16xlarge', max_nodes_ra3_16xl))
        cluster.run_xpx("event unset EtTestNodeType")
    ...
    def base_test_limit_burst_write_tps(self, cluster):
        """
            This test checks if we are able to limit bursting on ra3 clusters
            of a given size.
            It does the following:
                1. Sets up tables for bursting.
                2. Runs dmls on the tables and ensures they burst
                3. Set the event EtTestNodeType
                4. Runs dmls on the tables and ensures they DO NOT burst
                5. Unset the event EtTestNodeType (Part of health check)
                6. Runs dmls on the tables & ensures they burst (Health check)
        """
        relprefix = "limit_burst_write_tps"
        ...
        with self.setup_tables(cluster, cur1, relprefix, diststyle, sortkey,
            ...
            # All should burst since the limit is 1024 nodes. Any cluster
            # greater than or equal to 1024 nodes will not burst. Any cluster
            # smaller than that will burst and sim cluster is of 3 nodes.
            for size in [4, 1024]:
                cluster.run_xpx(NODE_TYPE.format('d2.rsxlarge', size))
                self._run_dmls(cluster, cur1, tables, True, True)
                cluster.run_xpx(NODE_TYPE.format('i3en.xlarge', size))
                self._run_dmls(cluster, cur1, tables, True, True)
                cluster.run_xpx(NODE_TYPE.format('i3en.3xlarge', size))
                self._run_dmls(cluster, cur1, tables, True, True)
                cluster.run_xpx(NODE_TYPE.format('i3en.12xlarge', size))
                self._run_dmls(cluster, cur1, tables, True, True)

            # The size of sim cluster is 3 and we limited burst at 3 nodes,
            # which means any ra3 cluster >= 3 nodes should not burst
            for size in [2, 1]:
                cluster.run_xpx(NODE_TYPE.format('i3en.xlarge', size))
                self._run_dmls(cluster, cur1, tables, True, False, 62)
                cluster.run_xpx(NODE_TYPE.format('i3en.3xlarge', size))
                self._run_dmls(cluster, cur1, tables, True, False, 62)
                cluster.run_xpx(NODE_TYPE.format('i3en.12xlarge', size))
                self._run_dmls(cluster, cur1, tables, True, False, 62)
...
# Simulated cluster is of 3 nodes.
@pytest.mark.localhost_only
@pytest.mark.super_simulated_mode
@pytest.mark.custom_burst_gucs(gucs=dict(list(burst_write_basic_gucs.items()) + [
                                ('slices_per_node', '3'),
                            ]))
...
@pytest.mark.custom_local_gucs(gucs=dict(list(burst_write_basic_gucs.items()) + [
            ...
            ('burst_write_max_num_nodes_ra3_xl', '16'),
            ('burst_write_max_num_nodes_ra3_4xl', '16'),
            ('burst_write_max_num_nodes_ra3_16xl', '16'),
                            ]))
class TestLimitBurstWriteTPSSS(TestLimitBurstWriteTPSBase):
    def test_limit_burst_write_tps_ss(self, cluster):
        self.base_test_limit_burst_write_tps(cluster)
