...
class BaseBurstCopyDistSortWithMoreSlice(BurstTempWrite):
    @classmethod
    ...
    def base_test_burst_copy_all_dist_sort_more_slice(
            self, cluster, vector, is_temp, unified_remote_exec):
        """
...
class TestBurstCopyDistSortWithMoreSlice(
        BaseBurstCopyDistSortWithMoreSlice):
    @classmethod
    def modify_test_dimensions(cls):
        return Dimensions(
            dict(
    ...
    def test_burst_copy_all_dist_sort_more_slice(self, cluster, vector,
                                                 is_temp):
        self.base_test_burst_copy_all_dist_sort_more_slice(
...
@pytest.mark.custom_burst_gucs(
    gucs=burst_unified_remote_exec_gucs_burst(slices_per_node=6))
@pytest.mark.custom_local_gucs(
    gucs=burst_unified_remote_exec_gucs_main(burst_use_ds_localization='true'))
class TestBurstCopyDistSortWithMoreSliceUnifiedRemoteExec(
        BaseBurstCopyDistSortWithMoreSlice):
    @classmethod
    def modify_test_dimensions(cls):
        return Dimensions(
    ...
    def test_burst_copy_all_dist_sort_more_slice_unified_remote_exec(
            self, cluster, vector):
        self.base_test_burst_copy_all_dist_sort_more_slice(
