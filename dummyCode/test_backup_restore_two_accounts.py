...
START_GUC = {
    ...
    "s3_backup_block_midfix": "simul/101"
}


@pytest.yield_fixture(scope='function')
def backup_restore_two_accounts_start_guc(request, cluster):
...
class TestLocalhostBackupRestoreTwoAccounts(BARTestSuite):
    """
    This test case verifies two backup formats and various cases.
    with block midfix changing
    steps in test case:
    ------------------
    ...
    change version to backup version to v2 and restore version to v1
                      with backup_midfix = ac1
    restore from  b1
    add table/rows
    take back up b2
    change version to backup version to v2 and restore version to v2
                      with backup_midfix = ac2
                      with snapshot_midfix = ac1
    restore from  b2
    add table/rows
    take back up b3
    change version to backup version to v1 and restore version to v2
                      with snapshot_midfix = ac2
    restore from  b3
    add table/rows
    take back up b4
    ...
    def _execute_test(self, cluster_session, cluster):
        ...
        cluster.set_padb_conf_value_unsafe(
            's3_snapshot_prefix_format_version', 2)
        cluster.set_padb_conf_value_unsafe(
            's3_backup_block_midfix', "simul/909")
        cluster.set_padb_conf_value_unsafe(
            's3_snapshot_block_midfix', "simul/101")
        ...
        cluster.set_padb_conf_value_unsafe(
            's3_snapshot_prefix_format_version', 2)
        cluster.set_padb_conf_value_unsafe(
            's3_snapshot_block_midfix', "simul/909")
    ...
    @pytest.mark.usefixtures("backup_restore_two_accounts_start_guc")
    def test_backup_restore_two_accounts(self, cluster_session, cluster):
