...
file_type_to_language = {
    ...
    ".po": "text", ".template" : "text", ".cmake" : "text", ".cfg" : "text", "" : "text"
}

def file_size_is_within_limit(file_path, max_size=8 * 1024 * 1024):  # 8 MB in bytes
...
def ingest(): 
    ...
    # Use multithreading to add texts
    #3510.4376544952393 second for 56097 nodes
    total_nodes = len(nodes)
    i = 0
    start_time = time.time()
    for node in nodes:
        vector_store.add_texts(texts = [node.text],
                           metadatas = [node.metadata],
                           ids = [i])
        i+=1
        print(f"Doing node {i} of {total_nodes}")


    end_time = time.time()
    
    print(f"It took {end_time - start_time} seconds to add all texts")
