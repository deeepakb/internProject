...
import time
import nmslib  # Importing nmslib
...
def ingest():
    ...
    reader = SimpleDirectoryReader(input_dir="/home/deeepak/redshift-padb", recursive=True)
    ...
    # Initialize nmslib vector store using CodeBERT embeddings
    ...
    for i, node in enumerate(nodes):
        ...
        # Adding embeddings to the nmslib index
        embedding = embeddings.embed_query(node["text"])
        index.addDataPoint(i, embedding)

        logger.info(f"Adding node {i + 1} of {total_nodes} to index")

    index.saveIndex('/home/deeepakb/Projects/bedrockTest/nmslib_index/codebase_index.nmslib')
    logger.info(f"Finished adding {total_nodes} nodes to nmslib index")

    # Example of querying the nmslib index (if needed)
    query_embedding = embeddings.embed_query("example query text")
    nearest_neighbors = index.knnQuery(query_embedding, k=5)
    logger.info(f"Nearest neighbors for query: {nearest_neighbors}")
